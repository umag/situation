...
use crate::api_models::{
    ...
    MergeStatusV1Response, // Added MergeStatusV1Response
    WhoamiResponse,
};
use dotenvy::dotenv;
...
) -> Result<(MergeStatusV1Response, Vec<String>), Box<dyn Error + Send + Sync>>
{
    ...
    if status.is_success() {
        let response_text = response.text().await?;
        logs.push(format!("API Success Body: {}", response_text));
        let merge_status_response: MergeStatusV1Response =
            serde_json::from_str(&response_text).map_err(|e| {
                format!(
...
/// - Handles success (200 OK, empty body) and error responses similarly to other API client functions.
/// - Logs relevant information about the request and response.
pub async fn force_apply_change_set(
    workspace_id: &str,
    change_set_id: &str,
